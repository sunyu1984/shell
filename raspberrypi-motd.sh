...
greetings="$greetings$borderBar$(color $greetingsColor "$(center "$(date +"%Y-%m-%d %H:%M:%S")")")$borderBar"

# System information
read loginFrom loginIP loginDate <<< $(last $me -2 | awk 'NR==2 { print $2,$3,$4" "$5" "$6" "$7 }')

# TTY login
if [[ $loginDate == - ]]; then
  loginDate=$loginIP
  loginIP=`echo $loginFrom | sed 's/[ \t]*$//g'`
fi

if [[ $loginDate == * ]]; then
  login="$(date -d "$loginDate" +"%Y-%m-%d %H:%M") ($loginIP)"
else
  # Not enough logins
  login="None"
fi
